...
void GraphTest::testBioBlocksJSONReader() {
    try {
        std::shared_ptr<ProtocolGraph> trasnlated(BioBlocksJSONReader::GetInstance()->loadFile("bioBlocksProtocol.json"));
        trasnlated->printProtocol("trasnlatedProtocol.graph");
        QVERIFY(true);
